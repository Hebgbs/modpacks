...
function chgState {
  if ( $modMode -eq 1 ) {
    if ( -not ( Test-Path -Path "$modPath\BepInEx\$pMgrDes" ) ) {
      mkdir $modPath\BepInEx\$pMgrDes | Out-Null
    }
    if ( -not ( Test-Path -Path "$modPath\BepInEx\$cMgrDes" ) ) {
      mkdir $modPath\BepInEx\$cMgrDes | Out-Null
    }
  }
  if ( Test-Path -Path "$modPath\BepInEx\$pMgrDes\$modChk" ) {
  ...
  if ( $modPf.dll -ne 0 ) {
    if ( ( $modMode -eq 0 ) -and
       ...
       ( -not ( Test-Path -Path "$modPath\BepInEx\$pMgrDes\$modPf.dll" ) ) ) {
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$pMgrDes\$modPf.dll" -ItemType SymbolicLink -Path "$modPath\BepInEx\$pMgrDes\$modPf.dll" -ErrorAction SilentlyContinue | Out-Null
    }
    if ( Test-Path -Path "$modPath\BepInEx\$pMgrSrc\$modPf.dll" ) {
      Move-Item -Path "$modPath\BepInEx\$pMgrSrc\$modPf.dll" -Destination "$modPath\BepInEx\$pMgrDes\$modPf.dll"
    }
  }
  if ( $modPd -ne 0 ) {
    if ( ( $modMode -eq 0 ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$pMgrSrc\$modPd" ) ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$pMgrDes\$modPd" ) ) ) {
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$pMgrDes\$modPd" -ItemType Junction -Path "$modPath\BepInEx\$pMgrDes\$modPd" -ErrorAction SilentlyContinue | Out-Null
    }
    if ( Test-Path -Path "$modPath\BepInEx\$pMgrSrc\$modPd" ) {
      Move-Item -Path "$modPath\BepInEx\$pMgrSrc\$modPd" -Destination "$modPath\BepInEx\$pMgrDes\$modPd"
    }
  }
  if ( $modCf.cfg -ne 0 ) {
    if ( ( $modMode -eq 0 ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$cMgrSrc\$modCf.cfg" ) ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$cMgrDes\$modCf.cfg" ) ) ) {
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$cMgrDes\$modCf.cfg" -ItemType SymbolicLink -Path "$modPath\BepInEx\$cMgrDes\$modCf.cfg" -ErrorAction SilentlyContinue | Out-Null
    }
    if ( Test-Path -Path "$modPath\BepInEx\$cMgrSrc\$modCf.cfg" ) {
      Move-Item -Path "$modPath\BepInEx\$cMgrSrc\$modCf.cfg" -Destination "$modPath\BepInEx\$cMgrDes\$modCf.cfg"
    }
  }
  if ( $m2P -eq 3 ) {
    if ( ( $modMode -eq 0 ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$pMgrSrc\${modPf}four.dll" ) ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$pMgrDes\${modPf}four.dll" ) ) ) {
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$pMgrDes\${modPf}two.dll" -ItemType SymbolicLink -Path "$modPath\BepInEx\$pMgrDes\${modPf}two.dll" -ErrorAction SilentlyContinue | Out-Null
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$pMgrDes\${modPf}three.dll" -ItemType SymbolicLink -Path "$modPath\BepInEx\$pMgrDes\${modPf}three.dll" -ErrorAction SilentlyContinue | Out-Null
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$pMgrDes\${modPf}four.dll" -ItemType SymbolicLink -Path "$modPath\BepInEx\$pMgrDes\${modPf}four.dll" -ErrorAction SilentlyContinue | Out-Null
    }
    if ( Test-Path -Path "$modPath\BepInEx\$pMgrSrc\${modPf}four.dll" ) {
      Move-Item -Path "$modPath\BepInEx\$pMgrSrc\${modPf}two.dll" -Destination "$modPath\BepInEx\$pMgrDes\${modPf}two.dll"
      Move-Item -Path "$modPath\BepInEx\$pMgrSrc\${modPf}three.dll" -Destination "$modPath\BepInEx\$pMgrDes\${modPf}three.dll"
      Move-Item -Path "$modPath\BepInEx\$pMgrSrc\${modPf}four.dll" -Destination "$modPath\BepInEx\$pMgrDes\${modPf}four.dll"
    }
    if ( ( $modMode -eq 0 ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$cMgrSrc\${modCf}four.cfg" ) ) -and
       ( -not ( Test-Path -Path "$modPath\BepInEx\$cMgrDes\${modCf}four.cfg" ) ) ) {
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$cMgrDes\${modCf}two.cfg" -ItemType SymbolicLink -Path "$modPath\BepInEx\$cMgrDes\${modCf}two.cfg" -ErrorAction SilentlyContinue | Out-Null
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$cMgrDes\${modCf}three.cfg" -ItemType SymbolicLink -Path "$modPath\BepInEx\$cMgrDes\${modCf}three.cfg" -ErrorAction SilentlyContinue | Out-Null
      New-Item -Target "$repoPath\$repoBranch\BepInEx\$cMgrDes\${modCf}four.cfg" -ItemType SymbolicLink -Path "$modPath\BepInEx\$cMgrDes\${modCf}four.cfg" -ErrorAction SilentlyContinue | Out-Null
    }
    if ( Test-Path -Path "$modPath\BepInEx\$cMgrSrc\${modCf}four.cfg" ) {
      Move-Item -Path "$modPath\BepInEx\$cMgrSrc\${modCf}two.cfg" -Destination "$modPath\BepInEx\$cMgrDes\${modCf}two.cfg"
      Move-Item -Path "$modPath\BepInEx\$cMgrSrc\${modCf}three.cfg" -Destination "$modPath\BepInEx\$cMgrDes\${modCf}three.cfg"
      Move-Item -Path "$modPath\BepInEx\$cMgrSrc\${modCf}four.cfg" -Destination "$modPath\BepInEx\$cMgrDes\${modCf}four.cfg"
    }
  }
  chgFin
}
...
function modMgmt {
  ...
  echo ""
    ...
    $pMgrSrc = "plugins"
    $pMgrDes = "disabledPlugins"
    $cMgrSrc = "config"
    $cMgrDes = "disabledConfig"
    modSel
	}
	if ( $m1P -eq 2 ) {
		consent
